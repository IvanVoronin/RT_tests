...
Тест продолжится через %d секунд' %(i),
    ...
    def wait(self, trial):
        self.trial_stimuli['fixation'].setLineColor('red')
#        self.trial_stimuli['fixation'].draw()
        ...
        self.test_screen.flip()
        self.keyboard.waitForPresses(keys=[self.interruptkey] + self.keys, 
                                     clear=True, maxWait=5)
        
        pressed = self.keyboard.state.keys()
        if len(pressed) == 0:
            self.instr_stimuli['instr_start'].draw()
            self.wait(trial)
    ...
    def run_trial(self, trial):
        self.trial_stimuli['fixation'].setLineColor('green')
        self.trial_stimuli['fixation'].draw()
        ...
        self.show_trial_screen()
        self.test_screen.flip()
        wait_stim = self.keyboard.waitForReleases(maxWait=wait_time, 
                                                  keys=self.keys, 
                                                  clear=True)
        ...
        if wait_stim:
            ...
            rel = self.keyboard.waitForReleases(keys=self.keys, 
